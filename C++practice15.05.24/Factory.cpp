...
void Factory_Staff::make_staff() {
	...
	MakeProjects();
	std::string first_name, second_name, third_name, position, other_information;
	...
	while(ifs >> id_person){
		...
		case Positions::tester:
		{
			...
			projects[id_proj]->SetTimeInProject(projects[id_proj]->GetTimeInProject() +
				worktime);
		...
		{
			ifs >> id_proj;
			TeamLeader* team_leader = new TeamLeader(id_person, first_name + " " +
				second_name + " " + third_name,
				worktime, salary, SetPosition(position),
				projects[id_proj]);
			projects[id_proj]->SetTimeInProject(projects[id_proj]->GetTimeInProject() +
		...
		{
			ifs >> id_proj;
			ProjectManager* project_manager = new ProjectManager(id_person, first_name + " " +
				second_name + " " + third_name,
				worktime, salary, SetPosition(position), projects[id_proj]);
			projects[id_proj]->SetTimeInProject(projects[id_proj]->GetTimeInProject() + worktime);
		...
		case Positions::senior_manager:
		{
			...
			while (ss >> id_proj) {
				...
				projects[id_proj]->SetTimeInProject(projects[id_proj]->GetTimeInProject() + work_times[i++]);
			}
			SeniorManager* senior_manager = new SeniorManager(id_person, first_name + " " +
				second_name + " " + third_name,
				work_times, salary, SetPosition(position),
